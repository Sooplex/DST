...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
                      ...
                      unconditional_guidance_scale=1., unconditional_conditioning=None,features_adapter=None,#parameters for adapters
    ...
    def p_sample_ddim(self, x, c, t, index, negative_conditioning=None,
                      ...
                      features_adapter=None,#for adapters
                      append_to_context=None
                      ):
        b, *_, device = *x.shape, x.device

        if negative_conditioning is not None:#TODO:make it compatible with adapters
            x_in = torch.cat([x] * 2)
            t_in = torch.cat([t] * 2)
