...
#from adapters
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   ...
                   plot_diffusion_rows=True,unconditional_guidance_scale=1., unconditional_guidance_label=None,use_ema_scope=True, **kwargs):
    ...
    def forward(self,
        ...
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(x,
                                       t,
                                       injected_features=injected_features context=cc,**kwargs)
            ...
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc,injected_features=injected_features, y=c_adm, **kwargs)
        elif self.conditioning_key == 'crossattn-adm':
            assert c_adm is not None
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc,injected_features=injected_features, y=c_adm, **kwargs)
