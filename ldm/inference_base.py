...
def get_base_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default='/home/pnp/SINE/SINE-master/v1-5-pruned-emaonly.ckpt',
...
def diffusion_inference(opt, model, sampler, adapter_features, append_to_context=None,mask_pixel=None,img_raw=None,injected_features=None,ddim_steps=None):
    # get text embedding
    ...
    if mask_pixel is not None:
        #传给ddim的mask
        mask_latent = cv2.resize(mask_pixel, (opt.W // opt.f,opt.H // opt.f), interpolation=cv2.INTER_AREA)
        mask = 1.0 - torch.from_numpy(mask_latent.copy()).float().cuda()
        mask = einops.rearrange(mask, 'h w -> 1 h w').clone()
    if img_raw is not None :
        #x0各像素单位化为正态分布N（0，1）范围  
        x0 = torch.from_numpy(img_raw.copy()).float().cuda() / 127.0 - 1.0
        x0 = einops.rearrange(x0, 'h w c -> 1 c h w').clone()
        x0 = model.get_first_stage_encoding(model.encode_first_stage(x0))
    samples_latents, _ = sampler.sample(
        S=ddim_steps if ddim_steps is not None else opt.steps,
        conditioning=c,
        batch_size=1,
        shape=shape,
        verbose=False,
